...
describe('sms endpoint', () => {
  ...
  describe('POST /v1/sms/check', () => {
    ...
    it('验证码 action 不正确时返回 400', async () => {
      await SMSCode.clearTimestamp(mobile1)
      const code = await SMSCode.generateCode(mobile1, 'test')
      await request.post('/v1/sms/check')
        .send({mobile: mobile1, code})
        .expect(400)
    })
    it('验证码正确时返回 200', async () => {
      await SMSCode.clearTimestamp(mobile2)
      const code = await SMSCode.generateCode(mobile2, 'test')
      await request.post('/v1/sms/check')
        .send({mobile: mobile2, code, action: 'test'})
        .expect(200)
    })
  })
})
